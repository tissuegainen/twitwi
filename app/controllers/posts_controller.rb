class PostsController < ApplicationController
  ...
  def make_picture(id)
    ...
    case Rails.env
      when 'production'
        # ⑨-14 バケットの指定・URLの設定
        bucket = storage.directories.get('twitwi-production')
        ...
        @post.picture = 'https://s3-ap-northeast-1.amazonaws.com/twitwi-production' + "/" + png_path
      when 'development'
        bucket = storage.directories.get('twitwi-development')
        png_path = 'images/' + id.to_s + '.png'
        image_uri = image.path
        file = bucket.files.create(key: png_path, public: true, body: open(image_uri))
        @post.picture = 'https://s3-ap-northeast-1.amazonaws.com/twitwi-development' + "/" + png_path
    end
  end
end
